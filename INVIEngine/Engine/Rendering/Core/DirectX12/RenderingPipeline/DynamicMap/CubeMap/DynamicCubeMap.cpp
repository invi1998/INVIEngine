		...
		for (int j = 0; j < GeometryMap->DynamicReflectionMeshComponents.size(); j++)
		{
			...
			for (size_t i = 0; i < 6; i++)
			{
				...
				GetD3dGraphicsCommandList()->ClearRenderTargetView(
					inRenderTarget->CPURenderTargetView[i],		// 要清除的渲染目标视图
					ColorBG,		// 画布颜色
				...
				GetD3dGraphicsCommandList()->OMSetRenderTargets(
					1,									// 指定渲染目标数 1
					&inRenderTarget->CPURenderTargetView[i],		// 指定渲染目标
			...
			RenderLayers->FindObjectDraw(DeltaTime, RENDER_LAYER_OPAQUE_REFLECT, GeometryMap->DynamicReflectionMeshComponents[j]);
