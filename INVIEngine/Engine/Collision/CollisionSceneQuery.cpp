...
void FCollisionSceneQuery::GetRaycastParam(std::shared_ptr<FRenderingData>& renderData, const XMVECTOR& originPoint,
	const XMVECTOR& direction, const XMMATRIX& viewInverseMatrix, XMVECTOR& outOriginPoint, XMVECTOR& outDirection)
{
	// 获取模型的世界矩阵
	...
	outOriginPoint = originPointVW;
	outDirection = directionVW;
}
...
bool FCollisionSceneQuery::RayCastSingleQuery(CWorld* world, const XMVECTOR& origin, const XMVECTOR& direction, const XMMATRIX& viewInverseMatrix, EngineType::FHitResult& OutHitResult)
{
	...
	for (auto& renderData : FGeometry::RenderingDataPoolVector)
	{
		...
		XMMATRIX worldMatrix = XMLoadFloat4x4(&renderData->WorldMatrix);
		XMVECTOR originPointVW = {};	// 模型空间下的射线原点坐标
		XMVECTOR directionVW = {};		// 模型空间下的射线方向

		GetRaycastParam(renderData, origin, direction, viewInverseMatrix, originPointVW, directionVW);
	...
	const XMMATRIX& viewInverseMatrix, EngineType::FHitResult& OutHitResult, const GActorObject* actor, std::vector<CComponent*>& ignoreComponents)
{
	...
	for (auto& renderData : FGeometry::RenderingDataPoolVector)
	{
		...
		if (renderData->Bounds.Intersects(originPointVW, directionVW, boundTime))
		{
			if (GActorObject* inActorObject = dynamic_cast<GActorObject*>(renderData->Mesh->GetOwner()))
			{
				if (inActorObject == actor)
				{
					OutHitResult.bHit = true;
					OutHitResult.Time = boundTime;
					OutHitResult.HitComponent = renderData->Mesh;
					OutHitResult.HitRenderingData = renderData;
					OutHitResult.HitActor = inActorObject;
					return true;
				}
			}
		}
