...
在计算机图形学中，切线空间和世界空间是两个常用的坐标系。其中，切线空间通常用于计算法线贴图、阴影和反射等效果，而世界空间则用于描述物体在三维空间中的位置和姿态。

下面我们介绍如何将一个向量从切线空间转换到世界空间，以及如何将一个向量从世界空间转换到切线空间。

1. 从切线空间转换到世界空间

假设有一个三角形网格，在顶点处存储了位置向量 `p`、法向量 `n` 和切向量 `t`，并且在纹理坐标系中存储了每个顶点的副法向量 `b` 和切向量 `T`。如果需要将一个向量从切线空间（即纹理空间）转换到世界空间，则可以使用以下步骤：
...
- 将向量从切线空间转换到世界空间
假设要将一个在切线空间中的向量 `(tx, ty, tz)` 转换到世界空间中的向量 `(wx, wy, wz)`，则可以将其乘以上述变换矩阵得到：
...
同样地，如果需要将一个向量从世界空间转换到切线空间，则可以使用以下步骤：

- 计算变换矩阵
首先需要计算一个变换矩阵，将切向量 `T`、副法向量 `b` 和法向量 `n` 转换为世界空间中的向量。该变换矩阵的逆矩阵为：
...
- 将向量从世界空间转换到切线空间
假设要将一个在世界空间中的向量 `(wx, wy, wz)` 转换到切线空间中的向量 `(tx, ty, tz)`，则可以将其乘以上述变换矩阵的逆矩阵得到：
...
需要注意的是，在实际应用中，可以使用一些第三方库（例如 Eigen、OpenCV 等）来进行坐标空间之间的转换
...
![image-20231013165049661](.\image-20231013165049661.png)
